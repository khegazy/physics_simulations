...
OUTDIR=/cds/group/ued/scratch/N2O/simulations/
...
do
  ...
  do
    lmk=$j",0,"$k
    echo "Submitting "${lmk}
    sbatch -p psanaq -o ${OUTPUTDIR}"job_"${lmk}".log" --wrap="python3 ~/simulation/diffractionSimulation/diffraction.py --run sim_validation --molecule CF2IBr --LMK $lmk --xyz_file CF2IBr.xyz --basis_folder /cds/group/ued/scratch/N2O/axis_distributions/NO2/A/temp-100K --time_ind 0 --cluster True"
